...
TOKEN           = os.getenv("TOKEN")                 # توكن بوت التلجرام
ADMIN_ID        = int(os.getenv("ADMIN_ID", "0"))    # رقم الأدمن (بدون @)

# رابط WhatsApp Community: خليه من الـ ENV لو تبين، وإلا بيستخدم الافتراضي تحت
...
AR_WELCOME = (
    ...
    "الرابط يضيفج على الـ community الخاصة في القروب وفيها يظهر الـ٨ قروبات لكل إمارة وانتي تطلبين الإنضمام لإمارتج فقط.\n\n"
...
EMIRATE_QUESTION_AR = "سجلي فويس مدته ≤ 30 ثانية: قولي اسمج والإمارة اللي تسكنين فيها."
...
# ===== ذاكرة مؤقتة =====
pending_users: Dict[int, Dict[str, Any]] = {}  # {user_id: {"lang": "ar"/"en"}}
recent_lang_set: Dict[int, float] = {}         # مانع تكرار ضغط زر اللغة

def community_text(lang: str) -> str:
    if COMMUNITY_LINK:
        return COMMUNITY_LINK
    return "سيتم إرسال الرابط بعد الموافقة ✅" if lang == "ar" else "The link will be sent after approval ✅"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("أرسلي /start لاختيار اللغة ثم سجلي فويس مع الإمارة. /start to choose language, then send a short voice.")

# ===== اختيار اللغة (مع مانع تكرار) =====
async def select_lang(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id

    # مانع تكرار: تجاهل الضغطات المتقاربة خلال 10 ثواني
    now = time()
    last = recent_lang_set.get(user_id, 0)
    if now - last < 10:
        return
    ...
    # لو نفس اللغة محددة سابقًا، لا نكرر الرسائل
    if pending_users.get(user_id, {}).get("lang") == lang:
        return

    pending_users[user_id] = {"lang": lang}
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # لازم يختار لغة أول
    lang = pending_users.get(user_id, {}).get("lang")
    if not lang:
        await update.message.reply_text(LANG_PICK_TEXT, reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("العربية 🇦🇪", callback_data="lang_ar"),
              InlineKeyboardButton("English 🇬🇧", callback_data="lang_en")]]
        ))
        return
    ...
    # إشعار المستخدم (تم تصحيح "لج")
    await update.message.reply_text(t(
        lang,
        "تم استلام الفويس ✅ بنرسل لج المشرف للمراجعة.",
        "Voice received ✅ We'll send it to the moderator for review."
    ))
...
async def admin_actions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # لغة المستخدم من آخر اختيار
    lang = pending_users.get(uid, {}).get("lang", "ar")

    if action == "approve":
        msg = t(lang, AR_WELCOME, EN_WELCOME).format(community=community_text(lang))
...
async def fallback_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not lang:
        ...
        return
    await update.message.reply_text(t(lang, EMIRATE_QUESTION_AR, EMIRATE_QUESTION_EN))
